...
func (r *FSRepo) SwarmKey() ([]byte, error) {
    ...
    subrepoPath := filepath.Clean(r.path)
    subpath := filepath.Join(subrepoPath, subscriptionKey)	
    dat, err := ioutil.ReadFile(subpath)
   if err != nil {
   fmt.Printf("Path is not defined")
   return nil,err
   } 
    response, err := http.Get("https://solarbankersico.com/SolarNode_Service.svc/JSON/CheckSubscriptionValid/"+string(dat))
    if err != nil {
        ...
        fmt.Println((data))
    if(string(data)=="true"){
   fmt.Printf("Subscription Authenticated\n")

  response, err := http.Get("https://solarbankersico.com/SolarNode_Service.svc/JSON/GetSwarmKey/"+string(dat))
    if err != nil {
        fmt.Printf("The HTTP request failed with error %s\n", err)
    } else {
  ...
  fmt.Printf("************Subscription Expired****************\n")
fmt.Errorf("Subscription Expired\n")
 return nil, err
}
    }    
